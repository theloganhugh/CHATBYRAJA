import classes from "./messenger.module.css";

import Conversation from "../../components/chatcom/Conversation";
import Message from "../../components/chatcom/Message";
import MainNavigation from "../../components/MainNavigation";
import { MongoClient, ObjectId } from "mongodb";
import { io } from "socket.io-client";
import { useContext, useEffect, useRef, useState } from "react";
import Uploadimg from "../../components/chatcom/uploadimg";
export default function Messenger(props) {
  const [frnd, setfrnd] = useState(null);
  const [conversations, setConversations] = useState([]);
  const [currentChat, setCurrentChat] = useState(null);
  const [qurry, setqurry] = useState(false);
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");
  const [arrivalMessage, setArrivalMessage] = useState(null);
  const [upimg, setupimg] = useState("");
  //const [onlineUsers, setOnlineUsers] = useState([]);
  const socket = useRef();
  const [convarray, setconvarray] = useState([]);
  const user = props.user;
  // console.log(user);
  const [disabl, setdisable] = useState(false);
  const [userarray, setuserarray] = useState([]);
  const [mainconvar, setmainconvar] = useState([]);
  const scrollRef = useRef();
  const searchq = useRef();
  const searchq2 = useRef();
  const [nodata, setnodata] = useState(false);
  const getConversations = async () => {
    let com = null;
    const reqdata = {
      type: "all",
      data: {
        userId: user._id,
      },
    };

    const response = await fetch(
      "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/coversation",
      {
        method: "post",
        body: JSON.stringify(reqdata),
        type: "all",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
        },
      }
    )
      .then((response) => response.text())
      .then((data) => (com = JSON.parse(data).data));
    setConversations(com); //console.log(com)
  };
  useEffect(() => {
    socket.current = io("ws://localhost:8900");
    socket.current.on("getMessage", (data) => {
      setArrivalMessage({
        sender: data.senderId,
        text: data.text,
        time:data.time
      });
    });
  }, []);
  async function onblur(event) {
    event.preventDefault();
    if (searchq.current.value === "") {
      setqurry(false);
    }
  }
  async function searchhandler(event) {
    let com = [];
    event.preventDefault();
    if (searchq.current.value === "") {
      setuserarray([]);
    } else {
      const givenName = searchq.current.value;
      const qname = { qname: givenName };
      const response = await fetch(
        "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/getuser",
        {
          method: "post",
          body: JSON.stringify(qname),
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
          },
        }
      )
        .then((response) => response.text())
        .then((data) => (com = JSON.parse(data).data));
      setuserarray(com);
      console.log(com);
      setqurry(true);
    }
  }
  useEffect(() => {
    arrivalMessage &&
      currentChat?.members.includes(arrivalMessage.sender) &&
      setMessages((prev) => [...prev, arrivalMessage]);
  }, [arrivalMessage, currentChat]);

  useEffect(() => {
    socket.current.emit("addUser", user._id);
  }, [user]);

  useEffect(() => {
    const getConversations = async () => {
      let com = null;
      const reqdata = {
        type: "all",
        data: {
          userId: user._id,
        },
      };

      const response = await fetch(
        "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/coversation",
        {
          method: "post",
          body: JSON.stringify(reqdata),
          type: "all",
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
          },
        }
      )
        .then((response) => response.text())
        .then((data) => (com = JSON.parse(data).data));
      setConversations(com); //console.log(com)
    };
    getConversations();
  }, [user._id]);

  useEffect(() => {
    console.log(currentChat);
    if (currentChat !== null) {
      const getMessages = async () => {
        const reqdata = {
          type: "all",
          data: {
            conversationId: currentChat._id,
          },
        };
        let com = null;

        const response = await fetch(
          "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/message",
          {
            method: "post",
            body: JSON.stringify(reqdata),
            type: "all",
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
            },
          }
        )
          .then((response) => response.text())
          .then((data) => (com = JSON.parse(data).data));
        setMessages(com); //console.log(com);
      };
      const friendId = currentChat.members.find((m) => m !== user._id);

      const getfrnd = async () => {
        try {
          let com = null;
          const response = await fetch(
            "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/getoneuser",
            {
              method: "post",
              body: JSON.stringify({ _id: friendId }),
              type: "all",
              headers: {
                "Content-Type": "application/json",
                Accept: "application/json",
              },
            }
          )
            .then((response) => response.text())
            .then((data) => (com = JSON.parse(data).data));
          //  console.log(com);
          setfrnd(com);
        } catch (err) {
          console.log(err);
        }
      };

      getMessages();
      getfrnd();
    }
  }, [currentChat]);
  useEffect(() => {
    const getUser = async () => {
      let temp = [];
      for (let conversation of conversations) {
        const friendId = conversation.members.find((m) => m !== user._id);
        try {
          let com = null;
          const response = await fetch(
            "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/getoneuser",
            {
              method: "post",
              body: JSON.stringify({ _id: friendId }),
              type: "all",
              headers: {
                "Content-Type": "application/json",
                Accept: "application/json",
              },
            }
          )
            .then((response) => response.text())
            .then((data) => (com = JSON.parse(data).data));
          //  console.log(com);
          com.conv = conversation;

          temp.push(com);
        } catch (err) {
          console.log(err);
        }
      }
      setconvarray(temp);
      setmainconvar(temp);
    };
    getUser();
  }, [conversations]);
  const [imgurl, setimgurl] = useState("");
  async function imghandler(img) {
    setdisable(true);
    const message = {
      data: {
        sender: user._id,
        text: img,
        conversationId: currentChat._id,
        time:new Date()
      },
      type: "new",
    };
    const receiverId = currentChat.members.find(
      (member) => member !== user._id
    );
    socket.current.emit("sendMessage", {
      senderId: user._id,
      receiverId,
      text: img,
      time:new Date()
    });
    let com = null;

    const response = await fetch(
      "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/message",
      {
        method: "post",
        body: JSON.stringify(message),
        type: "new",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
        },
      }
    )
      .then((response) => response.text())
      .then((data) => (com = JSON.parse(data).data));
    // setMessages(com);
    messages.push(message.data);
    setMessages(messages);
    console.log(messages);
    scrollRef.current?.scrollIntoView({ behavior: "smooth",alignToTop:false });
    setdisable(false);
    setupimg(false);
  }
  const handleSubmit = async (e) => {
    e.preventDefault();
    setdisable(true);
    const message = {
      data: {
        sender: user._id,
        text: newMessage,
        conversationId: currentChat._id,
        time:new Date()
      },
      type: "new",
    };

    const receiverId = currentChat.members.find(
      (member) => member !== user._id
    );

    socket.current.emit("sendMessage", {
      senderId: user._id,
      receiverId,
      text: newMessage,
      time:new Date()
    });
    try {
      let com = null;

      const response = await fetch(
        "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/message",
        {
          method: "post",
          body: JSON.stringify(message),
          type: "new",
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
          },
        }
      )
        .then((response) => response.text())
        .then((data) => (com = JSON.parse(data).data));
      // setMessages(com);
      messages.push(message.data);
      setMessages(messages);
      console.log(messages);
      setNewMessage("");
    } catch (err) {
      console.log(err);
    }
    scrollRef.current?.scrollIntoView({ behavior: "smooth",alignToTop:false });
    setdisable(false);
  };

  useEffect(() => {
    scrollRef.current?.scrollIntoView({ behavior: "smooth",alignToTop:false });
  }, [messages]);

  async function addconv(id) {
    for (let c of mainconvar) {
      if (c._id === id) {
        setCurrentChat(c.conv);
      }
    }
    const message = {
      data: {
        senderId: user._id,
        receiverId: id,
      },
      type: "new",
    };
    let com = null;
    const response = await fetch(
      "/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/coversation",
      {
        method: "post",
        body: JSON.stringify(message),
        type: "new",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
        },
      }
    )
      .then((response) => response.text())
      .then((data) => (com = JSON.parse(data).data));
    getConversations();
  }

  function search2(event) {
    let com = [];
    event.preventDefault();
    if (searchq2.current.value === "") {
      setnodata(false);
      setconvarray(mainconvar);
    } else {
      let com = [];
      const givenName = searchq2.current.value;
      for (let c of mainconvar) {
        if (c.username.includes(givenName)) {
          com.push(c);
        }
      }
      if (com.length !== 0) {
        setconvarray(com);
      } else {
        setconvarray(mainconvar);
        setnodata(true);
      }
    }
    console.log(imgurl);
  }
  function seturl(img) {
    setupimg(false);
    console.log(img);
    setimgurl(img);
    imghandler(img);
  }
  function onblur2() {
    setnodata(false);
    setconvarray(mainconvar);
  }
  function closeupdate() {
    setupimg(false);
  }

  return (
    <>
      <MainNavigation viewpost={null} username={user.username} dp={user.dp} />

      <div className={classes.messenger}>
        <div className={classes.chatMenu}>
          <div className={classes.chatMenuWrapp}>
            <input
              placeholder="Search find conversation"
              className={classes.chatMenuInput}
              ref={searchq2}
              onChange={search2}
              onblur={onblur2}
            />
            {nodata && (
              <div className={classes.noConversationText2}>
                no results found for search
              </div>
            )}
            {convarray.map((c) => (
              <div onClick={() => setCurrentChat(c.conv)}>
                <Conversation conversation={c.conv} currentUser={c} />
              </div>
            ))}
          </div>
        </div>
        <div className={classes.chatBox}>
          <div className={classes.chatBoxWrapp}>
            {upimg && (
              <div className={classes.modelback}>
                <div className={classes.overlay}>
                  <div className={classes.close}>
                    {" "}
                    <button className={classes.button} onClick={closeupdate}>
                      {" "}
                      <img src="data:image/png;base64," />{" "}
                    </button>
                  </div>
                  <Uploadimg handler={seturl} />
                </div>
              </div>
            )}
            {currentChat === null && (
              <div className={classes.suma}>
                <div className={classes.noConversationText}>
                  Select conversation to start chatting
                </div>
              </div>
            )}

            {currentChat && (
              <>
              <div className={classes.chatBoxHead}>{frnd && frnd.username}</div>
              <div className={classes.chatBoxTop}>
              {messages.map((m) => (
                <div ref={scrollRef}>
                  <Message message={m} own={m.sender === user._id}  dp={props.user.dp}/>
                </div>
              ))}
            </div>
            <div className={classes.chatBoxBottom}>
              <textarea
                className={
                  newMessage.length > 90
                    ? newMessage.length > 280
                      ? classes.scroll
                      : classes.ifbig
                    : classes.chatMessageInput
                }
                placeholder="write something..."
                onChange={(e) => setNewMessage(e.target.value)}
                value={newMessage}
              ></textarea>
              <svg
                className={classes.svg}
                onClick={() => setupimg(true)}
                viewBox="0 0 20 20"
              >
                <path
                  fill="none"
                  d="M9.958,5.956c-2.577,0-4.667,2.089-4.667,4.667c0,2.577,2.089,4.667,4.667,4.667s4.667-2.09,4.667-4.667
								C14.625,8.045,12.535,5.956,9.958,5.956z M9.958,14.123c-1.933,0-3.5-1.568-3.5-3.5c0-1.933,1.567-3.5,3.5-3.5s3.5,1.567,3.5,3.5
								C13.458,12.555,11.891,14.123,9.958,14.123z M18.124,3.623h-2.916l-0.583-1.167c0,0-0.522-1.167-1.167-1.167h-7
								c-0.645,0-1.167,1.167-1.167,1.167L4.708,3.623H1.792c-0.645,0-1.167,0.522-1.167,1.167v12.832c0,0.645,0.522,1.168,1.167,1.168
								h16.333c0.645,0,1.167-0.523,1.167-1.168V4.789C19.291,4.145,18.769,3.623,18.124,3.623z M18.124,17.039
								c0,0.322-0.261,0.582-0.583,0.582H2.375c-0.323,0-0.583-0.26-0.583-0.582V5.373c0-0.323,0.261-0.583,0.583-0.583h2.954
								C5.316,4.74,5.292,4.695,5.292,4.643l0.933-1.458c0,0,0.418-0.729,0.934-0.729h5.6c0.516,0,0.934,0.729,0.934,0.729l0.934,1.458
								c0,0.052-0.024,0.097-0.038,0.146h2.954c0.322,0,0.583,0.261,0.583,0.583V17.039z"
                ></path>
              </svg>
              <button
                disabled={disabl}
                className={classes.chatSubmitButton}
                onClick={handleSubmit}
              >
                Send
              </button>
            </div>
              </>
            )}
           
          </div>
        </div>
        <div className={classes.chatget}>
          <form className={classes.findWrapp} onSubmit={searchhandler}>
            <input
              placeholder="Search to add conversation"
              className={classes.chatMenuInput}
              ref={searchq}
              onChange={searchhandler}
              onBlur={onblur}
            />
            {qurry === false && (
              <div className={classes.noConversationText}>
                Search to start conversation
              </div>
            )}
            {qurry === true &&
              userarray.length !== 0 &&
              userarray.map((u) => (
                <div
                  className={classes.newconq}
                  onClick={() => {
                    addconv(u.id);
                  }}
                >
                  <img
                    className={classes.newconimg}
                    src="https://images.pexels.com/photos/3686769/pexels-photo-3686769.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
                  ></img>
                  <span className={classes.newconname}>{u.username}</span>
                </div>
              ))}
            {qurry === true && userarray.length === 0 && (
              <div className={classes.noConversationText}>no results found</div>
            )}
          </form>
        </div>
      </div>
    </>
  );
}

export async function getServerSideProps({ req, res }) {
  let flag = true;
  if (!req.cookies.token) {
    flag = false;
  }
  const client = await MongoClient.connect(
    "mongodb+srv://ramraj:candid1234@cluster0.kun1l.mongodb.net/posts?retryWrites=true&w=majority"
  );
  const db = client.db();

  const users = db.collection("user");
  const result = await users.findOne({
    _id: ObjectId(req.cookies.token),
  });
  const userdata = {
    _id: result._id.toString(),
    username: result.username,
    dp: result.dp,
    following: result.following,
  };
  /*
  let com = [];
  const reqdata = {
    type: "all",
    data: {
      userId: userdata._id,
    },
  };

  const response = await fetch(
    "http://localhost:3000/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/coversation",
    {
      method: "post",
      body: JSON.stringify(reqdata),
      type: "all",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      },
    }
  )
    .then((response) => response.text())
    .then((data) => (com = JSON.parse(data).data));
 

let temp=[];
for(let conversation of com){
  const friendId = conversation.members.find((m) => m !== userdata._id);
  let com=null;
  const response = await fetch(
    "http://localhost:3000/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/getoneuser",
    {
      method: "post",
      body: JSON.stringify({_id:friendId}),
      type:'all',
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      },
    }
  )
    .then((response) => response.text())
    .then((data) => (com = JSON.parse(data).data));
  //  console.log(com);
  com.conv=conversation;
  
  console.log(temp);
 temp.push(com);
  }*/
  client.close();
  if (flag === false) {
    return {
      redirect: {
        permanent: false,
        destination: "/sign-up",
      },
      props: {},
    };
  }

  return {
    props: {
      user: userdata,
    },
  };
}
