
import styles from "../../styles/Home.module.css";
import Head from "next/head";
import { Fragment } from "react";
import Post from "../../components/postcomponents/Post";
import MainNavigation from "../../components/MainNavigation";
import { MongoClient , ObjectId} from "mongodb";
import Layout from "../../components/Layout";
import Newpostcom from "../../components/postcomponents/Newpostcom";
import { useState , useRef} from "react";
import { useRouter } from "next/router";
import Spinner from "../../components/Spinner";
import classes from "./style.module.css"
import flash from 'next-flash'; 

export default function Home(props) {
  
////console.log(props.user)
  const router = useRouter();
  const [page,setpage ]=useState('e');
  const [pagepostlist, setpagepostlist]=useState(props.postlist)
 const inputref=useRef();
  const [newpost, setnewpost] = useState( false );
  const [spinner, setspinner] = useState( false );
  const [nodata, setnodata]=useState(props.nodata);
  function viewposthandler() {
   
    const param = 'add-post';
    router.push(
        {
          pathname: `/`,
          query: {
            param
          }
        },
        `/?param=${param}`,
        {shallow: true}
    );
    setnewpost( !newpost);
  }
  async function addpostHandler(enteredpostData, relate) {
    relate=relate.toLowerCase().trim();
    setnewpost( !newpost);
    const relatearr=[];
     setspinner(true);
     if(relate.length!==0){
      let temp="";
     for(let c of relate){
        if(c!==" "){
          temp=temp+c;
        }
        else{
          temp=temp.trim();
          relatearr.push(temp);
          temp="";
        }
     }
     console.log(relatearr)
    }
    const reqdata={
      data:enteredpostData,
      relate:relatearr
    }
    const response = await fetch("/api/newpost", {
      method: "POST",
      body: JSON.stringify(reqdata),
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      },
    });
    const data = await response.json();
    //////console.log(data);
    
    router.push("/");
    setspinner(false);
  }
async function searchhandler (event){
  event.preventDefault();
  let com=[];
  const qdata=inputref.current.value;
  qdata=qdata.toLowerCase().trim();
  const reqdata={
    q:qdata
  }
  const response = await fetch("/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/explore", {
    method: "POST",
    body: JSON.stringify(reqdata),
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
    },
  })  .then((response) => response.text())
  .then((data) => (com = JSON.parse(data).data));
  setpagepostlist(com)
  if(com.length===0)
  {
    setnodata(true);
  }
  inputref.current.value=""
 }
 async function getpost(q){
  let com=[];
  const qdata=q;
  qdata=qdata.toLowerCase().trim();
  const reqdata={
    q:qdata
  }
  const response = await fetch("/api/$2a$12$oWojfZwP5WKjomp2DGgRneccwiIDDT7fWgChOh6hu9NBqksKIOw7G/explore", {
    method: "POST",
    body: JSON.stringify(reqdata),
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
    },
  })  .then((response) => response.text())
  .then((data) => (com = JSON.parse(data).data));
  setpagepostlist(com)
  if(com.length===0)
  {
    setnodata(true);
  }
  inputref.current.value=""
 }
  function closemodel (){
    router.push("/");
    setnewpost(false);
  }
  function passdata(q){
    
    getpost(q);
    setpage(q);
  }
  function backto(){
    setpagepostlist(props.postlist);
    setnodata(false);
    setpage('e')
  }
  return (
    <Fragment>
    <Head>
      <style>
      
@import url('https://fonts.googleapis.com/css2?family=Coda:wght@800&display=swap');
</style>
    </Head>
   
    <MainNavigation viewpost={viewposthandler} dp={props.user.dp} username={props.user.username}/>
    <div className={classes.nav}><form className={classes.sform} onSubmit={searchhandler}>
            <div className={classes.control}>
              <input
                className={classes.incom}
                type="text"
                placeholder="Search"
                ref={inputref}
                required
                id="search"
                onSubmit={searchhandler}
                //onChange={checkcom}
                //user={props.user}
              />
              <button
                className={classes.combtn}
                
                // disabled={combtn}
              >
                <svg className={classes.svg1} viewBox="0 0 20 20">
                  <path
                    fill="none"
                    d="M19.129,18.164l-4.518-4.52c1.152-1.373,1.852-3.143,1.852-5.077c0-4.361-3.535-7.896-7.896-7.896
								c-4.361,0-7.896,3.535-7.896,7.896s3.535,7.896,7.896,7.896c1.934,0,3.705-0.698,5.078-1.853l4.52,4.519
								c0.266,0.268,0.699,0.268,0.965,0C19.396,18.863,19.396,18.431,19.129,18.164z M8.567,15.028c-3.568,0-6.461-2.893-6.461-6.461
								s2.893-6.461,6.461-6.461c3.568,0,6.46,2.893,6.46,6.461S12.135,15.028,8.567,15.028z"
                  ></path>
                </svg>
              </button>
            </div>
          </form>
          <button className={
            page==='e'?
            classes.nbtn
            :
            classes.btn} onClick={backto }>Explore</button>
          <button className={
            page==='movie'?
            classes.nbtn
            :
            classes.btn} onClick={()=>passdata('movie')}>Movies</button>
          <button className={
            page==='music'?
            classes.nbtn
            :
            classes.btn}onClick={()=> passdata('music')}>Music</button>
          <button className={
            page==='sport'?
            classes.nbtn
            :
            classes.btn}onClick={()=> passdata('sport')}>Sports</button>
          <button className={
            page==='gameing'?
            classes.nbtn
            :
            classes.btn}onClick={()=> passdata('gameing')}>Gaming</button>
          <button className={
            page==='news'?
            classes.nbtn
            :
            classes.btn}onClick={()=> passdata('news')}>News</button>
          <button className={
            page==='learning'?
            classes.nbtn
            :
            classes.btn}onClick={()=> passdata('learning')}>Learning</button>
          <button className={
            page==='fashion'?
            classes.nbtn
            :
            classes.btn}onClick={()=> passdata('fashion')}>Fashion</button >
       
          </div>
    <Layout >
      {newpost&& !spinner && (
        <div className={styles.modelback}>
          <div className={styles.overlay}>
            <div className={styles.close} > <button className={styles.button} onClick={closemodel}> <img src='data:image/png;base64,' /> </button></div>
            <Newpostcom onaddpost={addpostHandler} close={closemodel} user={props.user} />
          </div>
        </div>
      )}
      {spinner && (
        <div className={styles.modelback}>
         
            <Spinner  />
         
        </div>
      )}
    
     
        <div className={styles.main}>
          {nodata &&
          <div className={classes.nodiv}>
          <div className={classes.noConversationText}>
          No data found
     
            </div>
                 <button className={classes.btn} onClick={backto }>Keep Exploring</button>
                 </div>
            }

         {!nodata &&<div className={styles.container}>
            { pagepostlist.map((post) => (
              <Fragment>
              <Post
                key={post.id}
                id={post.id}
                image={post.image}
                username={post.username}
                caption={post.caption}
                com={post.comments}
                relate={post.relate}
                public={post.public}
                user={post.user}
              />
              
              </Fragment>
            ))}
          </div>}
        </div>
    
    </Layout>
    </Fragment>
  );
}


export async function getServerSideProps(ctx) {
 // console.log(req)
let flag=true;
  if(!ctx.req.cookies.token){
    
flag=false;
}
  const client = await MongoClient.connect(
    "mongodb+srv://ramraj:candid1234@cluster0.kun1l.mongodb.net/posts?retryWrites=true&w=majority"
  );
  const db = client.db();
  let nodataflag=false;
  let postlist=[];
if(flag===true){  
const users=db.collection("user");
const result = await users.findOne({
  _id:ObjectId(ctx.req.cookies.token), 
});
const post =db.collection('posts');
const resultpost = await post.find({public:true}).toArray();

//console.log(resultpost);
if(resultpost.length===0)
{
  postlist={
    username:result.username,
    caption:"",
    image:'https://i.pinimg.com/originals/c9/22/68/c92268d92cf2dbf96e3195683d9e14fb.png'
  }
  nodataflag=true;
}
if(!nodataflag){

postlist=resultpost.reverse();

}
const userdata={
  username:result.username,
  dp:result.dp,
  following:result.following
}
client.close();
if(nodataflag===true ||postlist.length===0){
  return {
    props: {
      ...flash.get(ctx)||{info:""},
      postlist: postlist,
     user:userdata,
     nodata:true,
    },
   
  };
}


console.log(postlist)
  return {
    props: {
      ...flash.get(ctx)||{info:""},
     postlist: postlist.map((post) => ({
        caption: post.caption,
          image: post.image,
          id: post._id.toString(),
          username:post.username,
          comments:post.comments,
          user:userdata,
          relate:post.relate,
          public:post.public
      })), 
      user:userdata,
     nodata:false,
    }
    
  };
}
client.close(); 
if(flag===false){
   return {
    redirect: {
      permanent: false,
      destination: "/sign-up",
    },
    props:{},
  };
}


}

